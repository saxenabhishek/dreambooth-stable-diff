...
def swap_text(option, base):
    ...
    if option == "object":
        ...
        freeze_for = 30
        return [
            f"You are going to train `object`(s), upload 5-10 images of each object you are planning on training on from different angles/perspectives. You can use services like <a style='text-decoration: underline' target='_blank' href='https://www.birme.net/?target_width={resize_width}&target_height={resize_width}'>birme</a> for smart cropping. {mandatory_liability}:",
            """<img src="file/cat-toy.png" />""",
            f"You should name your concept with a unique made up word that has low chance of the model already knowing it (e.g.: `{instance_prompt_example}` here). Images will be automatically cropped to {resize_width}x{resize_width}.",
            freeze_for,
        ...
        return [
            f"You are going to train a `person`(s), upload 10-20 images of each person you are planning on training on from different angles/perspectives. You can use services like <a style='text-decoration: underline' target='_blank' href='https://www.birme.net/?target_width={resize_width}&target_height={resize_width}'>birme</a> for smart cropping. {mandatory_liability}:",
            """<img src="file/person.png" />""",
            f"You should name your concept with a unique made up word that has low chance of the model already knowing it (e.g.: `{instance_prompt_example}` here). Images will be automatically cropped to {resize_width}x{resize_width}.",
            freeze_for,
            prior_preservation_box_update,
        ]
    elif option == "style":
        instance_prompt_example = "trsldamrl"
        freeze_for = 10
        return [
            f"You are going to train a `style`, upload 10-20 images of the style you are planning on training on. You can use services like <a style='text-decoration: underline' target='_blank' href='https://www.birme.net/?target_width={resize_width}&target_height={resize_width}'>birme</a> for smart cropping. Name the files with the words you would like  {mandatory_liability}:",
            """<img src="file/trsl_style.png" />""",
            f"You should name your concept with a unique made up word that has low chance of the model already knowing it (e.g.: `{instance_prompt_example}` here). Images will be automatically cropped to {resize_width}x{resize_width}",
            freeze_for,
            gr.update(visible=False),
        ]
...
def train(*inputs):
    ...
    if (
        ...
        or type_of_thing == "style"
        or (type_of_thing == "person" and not experimental_face_improvement)
    ):
    ...
    if not remove_attribution_after:
        ...
        if os.path.exists("intraining.lock"):
            os.remove("intraining.lock")
...
def push(model_name, where_to_upload, hf_token, which_model, comes_from_automated=False):
    ...
    if where_to_upload == "My personal profile":
        ...
        response = requests.post(
            "https://huggingface.co/organizations/sd-dreambooth-library/share/SSeOwppVCscfTEzFGQaqpfcjukVeNrKNHX",
            headers=headers,
        )
...
"""
    ...
    api.create_commit(
        repo_id=model_id, operations=operations, commit_message=f"Upload the model {model_name}", token=hf_token
    )
    api.upload_folder(folder_path="output_model", repo_id=model_id, token=hf_token)
    api.upload_folder(folder_path="instance_images", path_in_repo="concept_images", repo_id=model_id, token=hf_token)
    if is_spaces:
        if not comes_from_automated:
            extra_message = "Don't forget to remove the GPU attribution after you play with it."
        else:
            extra_message = (
                "The GPU has been removed automatically as requested, and you can try the model via the model page"
            )
    ...
    return [
        gr.update(
            visible=True, value=f"Successfully uploaded your model. Access it [here](https://huggingface.co/{model_id})"
        ),
        gr.update(visible=True, value=["diffusers_model.tar", "model.ckpt"]),
    ]
...
with gr.Blocks(css=css) as demo:
    ...
    gr.Markdown(
        "Customize Stable Diffusion v1 or v2 (ⁿᵉʷ!) by giving it a few examples of a concept. Based on the [🧨 diffusers](https://github.com/huggingface/diffusers) implementation, additional techniques from [TheLastBen](https://github.com/TheLastBen/diffusers) and [ShivamShrirao](https://github.com/ShivamShrirao/diffusers)"
    )
    ...
    with gr.Row() as what_are_you_training:
        ...
        base_model_to_use = gr.Dropdown(
            label="Which base model would you like to use?",
            choices=["v1-5", "v2-1-512", "v2-1-768"],
            value="v1-5",
            interactive=True,
        )
    ...
    with gr.Row() as upload_your_concept:
        with gr.Column():
            thing_description = gr.Markdown(
                "You are going to train an `object`, please upload 5-10 images of the object you are planning on training on from different angles/perspectives. You must have the right to do so and you are liable for the images you use, example"
            )
            thing_experimental = gr.Checkbox(
                label="Improve faces (prior preservation) - can take longer training but can improve faces",
                visible=False,
                value=False,
            )
            thing_image_example = gr.HTML("""<img src="file/cat-toy.png" />""")
            things_naming = gr.Markdown(
                "You should name your concept with a unique made up word that has low chance of the model already knowing it (e.g.: `cttoy` here). Images will be automatically cropped to 512x512."
            )
        ...
        with gr.Column():
            ...
            for x in range(maximum_concepts):
                ...
                file_collection.append(
                    gr.File(
                        label=f"""Upload the images for your {ordinal(x+1) if (x>0) else ""} concept""",
                        file_count="multiple",
                        interactive=True,
                        visible=visible,
                    )
                ...
                with gr.Column(visible=visible) as row[x]:
                    concept_collection.append(
                        gr.Textbox(
                            label=f"""{ordinal(x+1) if (x>0) else ""} concept prompt - use a unique, made up word to avoid collisions"""
                        )
                    )
                    with gr.Row():
                ...
                if counter_add < len(buttons_collection):
                    button.click(
                        ...
                        [
                            ...
                            buttons_collection[counter_add - 1],
                            buttons_collection[counter_add],
                            is_visible[counter_add],
                            file_collection[counter_add],
                        ],
                        queue=False,
                    )
                    ...
                    button.click(
                        lambda: [gr.update(visible=True), gr.update(visible=True), gr.update(visible=False), True],
                        None,
                        [
                            row[counter_add],
                            file_collection[counter_add],
            ...
            for delete_button in delete_collection:
                if counter_delete < len(delete_collection) + 1:
                    delete_button.click(
                        lambda: [gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), False],
                        None,
                        [
                            file_collection[counter_delete],
                            row[counter_delete],
                            buttons_collection[counter_delete - 1],
                            is_visible[counter_delete],
                        ],
                        queue=False,
                    )
                counter_delete += 1
    ...
    with gr.Box(visible=False) as training_summary:
        ...
        training_summary_checkbox = gr.Checkbox(
            label="Automatically remove paid GPU attribution and upload model to the Hugging Face Hub after training",
            value=True,
            visible=is_advanced_visible,
        )
        training_summary_model_name = gr.Textbox(label="Name of your model", visible=True)
        training_summary_where_to_upload = gr.Dropdown(
            ["My personal profile", "Public Library"], value="My personal profile", label="Upload to", visible=True
        )
        training_summary_token_message = gr.Markdown(
            '[A Hugging Face write access token](https://huggingface.co/settings/tokens), go to "New token" -> Role : Write. A regular read token won\'t work here.',
            visible=True,
        )
    ...
    if is_shared_ui:
        training_ongoing = gr.Markdown(
            "## This Space only works in duplicated instances. Please duplicate it and try again!", visible=False
        )
    elif not is_gpu_associated:
        training_ongoing = gr.Markdown(
            "## Oops, you haven't associated your T4 GPU to this Space. Visit the Settings tab, associate and try again.",
            visible=False,
        )
    else:
        training_ongoing = gr.Markdown(
            "## Training is ongoing ⌛... You can close this tab if you like or just wait. If you did not check the `Remove GPU After training`, you can come back here to try your model and upload it after training. Don't forget to remove the GPU attribution after you are done. ",
            visible=False,
        )

    # Post-training UI
    completed_training = gr.Markdown(
        """# ✅ Training completed. 
    ### Don't forget to remove the GPU attribution after you are done trying and uploading your model""",
        visible=False,
    )
    ...
    type_of_thing.change(
        fn=swap_text,
        inputs=[type_of_thing, base_model_to_use],
        outputs=[thing_description, thing_image_example, things_naming, perc_txt_encoder, thing_experimental],
        queue=False,
        show_progress=False,
    )

    # Swap the base model
    base_model_to_use.change(
        fn=swap_text,
        inputs=[type_of_thing, base_model_to_use],
        outputs=[thing_description, thing_image_example, things_naming, perc_txt_encoder, thing_experimental],
        queue=False,
    ...
    for file in file_collection:
        ...
        file.change(
            fn=count_files,
            inputs=file_collection
            + [thing_experimental]
            + [base_model_to_use]
            + [type_of_thing]
            + [steps]
            + [perc_txt_encoder]
            + [swap_auto_calculated],
            outputs=[training_summary, training_summary_text],
            queue=False,
        )

    thing_experimental.change(
        fn=count_files,
        inputs=file_collection
        + [thing_experimental]
        + [base_model_to_use]
    ...
    if is_spaces:
        training_summary_checkbox.change(
            fn=checkbox_swap,
            inputs=training_summary_checkbox,
            outputs=[
    ...
    train_btn.click(
        ...
        + [training_summary_where_to_upload]
        + [training_summary_model_name]
        + [training_summary_checkbox]
        + [training_summary_token]
        + [type_of_thing]
        + [steps]
        + [perc_txt_encoder]
        + [swap_auto_calculated],
        outputs=[result, try_your_model, push_to_hub, convert_button, training_ongoing, completed_training],
        queue=False,
    )
    ...
    push_button.click(
        fn=push,
        inputs=[model_name, where_to_upload, hf_token, base_model_to_use],
        outputs=[success_message_upload, result],
        queue=False,
    )
    ...
    demo.load(
        fn=check_status,
        inputs=top_description,
        outputs=[top_description, try_your_model, push_to_hub, result, convert_button],
        queue=False,
        show_progress=False,
    )
